...
class postcar_api
{
    ...
    function post_data(): array
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] != 'POST') {
            ...
            $query = $this->conn->prepare('INSERT INTO post_cars(username, vehicle_model, vehicle_number, seating_capacity,filename,rent_per_day, image_destination, token) VALUE(?,?,?,?,?,?,?,?)');
            $query->bind_param("ssssssss", $this->username, $this->vehicle_model, $this->vehicle_number, $this->seating_capacity,$this->filename,$this->rent_per_day, $this->image_destination, $this->token);
                if ($query->execute()) {
